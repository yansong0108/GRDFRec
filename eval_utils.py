...
class CLIPScore:
    def __init__(self, device='cuda' if torch.cuda.is_available() else 'cpu'):
        self.model, _, _ = open_clip.create_model_and_transforms('ViT-H-14', pretrained="./laion2b-s32b-b79K/open_clip_pytorch_model.bin")
...
def calculate_inception_score_given_data(data4eval, cate4eval, model_path, num_classes,
    ...
    model = InceptionV3(model_path, num_classes, device=device).to(device)
...
class CompatibilityEvaluator:
    def __init__(self, ckpt_path="", device='cuda' if torch.cuda.is_available() else 'cpu'):
        self.clip, _, _ = open_clip.create_model_and_transforms('ViT-H-14', pretrained="./laion2b-s32b-b79K/open_clip_pytorch_model.bin")
        self.evaluator = FashionEvaluator(cnn_feat_dim=1024)
        self.evaluator.load_state_dict(torch.load(ckpt_path, map_location=device))
        self.device = device
